...
Standard library only. It opens no network socket and starts no subprocess.
Only `pack` writes a file, into your clone. You can read the whole thing in ten minutes, and you should,
...
Run it from inside your global-bank-account clone. $M2 is this module's folder.
    ...
    python $M2/tools/ctxmeter.py count --absolute pom.xml docs/glossary.md
    python $M2/tools/ctxmeter.py repo --absolute
    python $M2/tools/ctxmeter.py turns $M2/bundles/naive.txt --turns 8
    python $M2/tools/ctxmeter.py pack $M2/bundles/naive.txt      # one file to attach
...
#: The repository being measured is the one you are standing in, not this module.
REPO = Path.cwd()
...
# tokenizer - see calibration.md, beside this file, for the method, the measured
# error and the date.
...
#: Residual bias after fitting, by content class. Re-fitted on global-bank-account
#: (Java) on 22 Sep 2026: Java's deep indentation and long identifiers read high.
CALIBRATION = {"code": 0.76, "data": 0.92, "prose": 0.85, "config": 0.85,
               "instructions": 0.85, "prompt": 0.85, "tools": 0.92, "other": 0.83}
...
def always_on(attached: list[Path]) -> list[Item]:
    ...
    if scoped_dir.is_dir():
        relative = [str(p.relative_to(REPO)) for p in attached if p.is_relative_to(REPO)]
...
def measure(paths: list[Path], prompt: Path | None = None,
    ...
    if include_always_on:
        always = always_on(paths)
        sent = {i.label for i in always}   # attaching an always-on file sends it once
        items = always + [i for i in items if i.label not in sent] + tool_schemas()
...
FOOTER = """
  ---------------------------------------------------------------------------
  ctxmeter is an ESTIMATOR, not a tokenizer. See calibration.md, beside it, for
  its measured error against a real tokenizer on global-bank-account.
...
def show(items: list[Item], heading: str) -> None:
    ...
    for item in sorted(items, key=lambda i: -i.tokens):
        ...
        label = item.label if len(item.label) <= 46 else "..." + item.label[-43:]
        print("  %-46s %9d  %-12s %4.1f%%%s"
              % (label, item.tokens, item.kind, share, note))
...
def cmd_repo(args) -> int:
    ...
    paths = [p for p in sorted(REPO.rglob("*"))
             ...
             {".java", ".py", ".json", ".xml", ".yml", ".yaml", ".sql", ".md",
              ".txt", ".toml", ".cfg", ".properties"}]
    items = measure(paths)
...
def cmd_pack(args) -> int:
    """Join a bundle into one file, so you can attach exactly what was measured."""
    paths = load_bundle(Path(args.bundle))
    target = REPO / f"pack-{Path(args.bundle).stem}.txt"
    parts = [f"===== {p.relative_to(REPO)} =====\n{normalise(read(p))}" for p in paths]
    target.write_text("\n".join(parts), encoding="utf-8")
    print(f"  wrote {target.name}: {len(paths)} files. Attach it with #file:{target.name}")
    print("  It is not part of the repository. Delete it when the lab ends.")
    return 0
...
def main(argv=None) -> int:
    ...
    p_pack = sub.add_parser("pack", help="join a bundle into one file to attach")
    p_pack.add_argument("bundle")
    p_pack.set_defaults(func=cmd_pack)

    args = parser.parse_args(argv)
    if not (REPO / ".git").exists():
        print(f"ctxmeter: {REPO} is not a git clone. Run this from inside your",
              file=sys.stderr)
        print("  global-bank-account clone, so the bundle paths resolve.", file=sys.stderr)
    if not args.command:
        parser.print_help()
        return 2
